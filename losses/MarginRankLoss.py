...
class MixedLoss(nn.Module):
    ...
    def forward(self, prediction, target, unlabeled_results):
        self.rankloss = 0.0
        self.countloss = 0.0
        self.losses = {}
        self.countloss = self.countloss_criterion(prediction, target) / prediction.shape[0]

        if self.lamda == 0.0:
            self.losses.update({
                'total': self.countloss,
                'label': self.countloss,
                'unlabel': self.rankloss
            })
            return self.losses
        ...
        self.total_loss = self.countloss + self.lamda * self.rankloss
